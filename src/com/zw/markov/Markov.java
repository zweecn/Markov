...
public final class Markov extends Object{
	...
	public static final int A_NO_ACTION = 0x1;
	public static final int A_TERMINATE = 0x2;
	public static final int A_RE_DO = 0x3;
	public static final int A_REPLACE = 0x4;
	public static final int A_RE_COMPOSITE = 0x5;
...
//		System.out.println("In terminate, state.nextToDoActivity=" + state.getNextToDoActivity());
		BaseAction action = new BaseAction(state.getNextToDoActivity().getNumber(), 
	...
	public static MarkovAction replace(MarkovState state) {
		...
		Activity oldFaultActivity = state.getFaultActivity();
		int oldFaultActivityNumber = oldFaultActivity.getNumber();
		int oldFaultServiceNumber = oldFaultActivity.getBlindService().getNumber();
		FreeServiceFinder freeServiceFinder = new FreeServiceFinderImp();
		AtomService newFreeService = freeServiceFinder.nextFreeService(oldFaultActivity);
		if (newFreeService == null) {
			state = null;
			return null;
		}
		oldFaultActivity.setBlindService(newFreeService);
		freeServiceFinder.setServiceUsed(newFreeService);
		oldFaultActivity.setX(0);
		state.setActivity(oldFaultActivity);
		state.init();
		ReplaceAction action = new ReplaceAction(oldFaultActivityNumber, 
				Markov.A_REPLACE, oldFaultServiceNumber, newFreeService.getNumber());
...
//				stateAfter.getNextToDoActivity().getBlindService().getNumber());
		...
		if (stateAfter.getGlobalState() == Markov.S_FAILED) {
			...
			records.add(new MarkovRecord(stateAfter, stateAfter, noAction, 1, 0, timeCost));
			return records;
		}  else {  
			records = new ArrayList<MarkovRecord>();
...
//			System.out.println("stateAfter2=" + stateAfter2);
			...
			records.add(new MarkovRecord(state, stateAfter1, noAction, 
					stateAfter.getNextToDoActivity().getBlindService().getQos().getReliability(), 0, timeCost));
			records.add(new MarkovRecord(state, stateAfter2, noAction, 
					1 - stateAfter.getNextToDoActivity().getBlindService().getQos().getReliability(), 0, timeCost));
